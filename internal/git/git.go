...
func NewGitOptions(repoPath string) *GitOptions {
	// 如果没有指定路径，使用当前目录
	if repoPath == "" {
		repoPath = "."
	}
...
func GetCommitsBetween(fromDate, toDate time.Time, opts *GitOptions) ([]CommitInfo, error) {
	// 格式化日期为git log可接受的格式
...
func GetCommitsThisWeek(opts *GitOptions) ([]CommitInfo, error) {
	// 计算本周一和下周一的日期
	now := time.Now()
...
func GetCommitDetails(hash string, opts *GitOptions) (*CommitInfo, error) {
	// 获取提交的基本信息
		...
		if refNames != "" {
			...
			for _, ref := range refs {
				...
				if strings.Contains(ref, "refs/heads/") {
					...
					branches = append(branches, branch)
				} else if !strings.Contains(ref, "tag:") && !strings.HasPrefix(ref, "HEAD") {
					// 去除远程分支前缀
